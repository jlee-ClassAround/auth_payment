export interface TossPayment {
  ...
  approvedAt?: string; // 승인 일시 (ISO8601)
  useEscrow: boolean; // 에스크로 사용 여부
  lastTransactionKey?: string; // 마지막 거래 키 (최대 64자)
  cancels?: Cancel[]; // 결제 취소 이력
  card?: Card; // 카드 결제 정보
  virtualAccount?: VirtualAccount; // 가상계좌 정보
  mobilePhone?: MobilePhone; // 휴대폰 결제 정보
  giftCertificate?: GiftCertificate; // 상품권 결제 정보
  transfer?: Transfer; // 계좌이체 정보
  metadata?: Record<string, string>; // 추가 메타데이터
  receipt?: Receipt; // 영수증 정보
  checkout?: Checkout; // 결제창 정보
  easyPay?: EasyPay; // 간편결제 정보
  country?: string; // 결제 국가 (ISO-3166 국가 코드)
  failure?: Failure; // 결제 실패 정보
  cashReceipt?: CashReceipt; // 현금영수증 정보
...
export interface Cancel {
  ...
  cancelRequestId?: string; // 취소 요청 ID
}
...
export interface Card {
  ...
  interestPayer?: "BUYER" | "CARD_COMPANY" | "MERCHANT"; // 할부 수수료 부담 주체
}
